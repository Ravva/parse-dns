...
async function pickLiveDnsProductUrl(contains: string): Promise<string> {
  const indexXml = (await axios.get('https://www.dns-shop.ru/sitemap.xml', { timeout: 30000 }))
    .data as string;
...
function parseDnsHtml(url: string, html: string): DnsNormalized {
  ...
  const current = priceText
    ? Number.parseInt(priceText.replace(/\s/g, '').replace(/\D/g, ''), 10)
    : null;
  const old = oldPriceText
    ? Number.parseInt(oldPriceText.replace(/\s/g, '').replace(/\D/g, ''), 10)
    : null;
  ...
  const availabilityText =
    $('.product-buy__availability').text().trim() ||
    $('[data-role="availability"]').text().trim() ||
    null;
  ...
  return {
    ...
    price: {
      ...
      old: Number.isFinite(old) ? old : null,
    },
    brand,
    availabilityText,
    specifications,
...
function parseDnsMicrodataResponse(microText: string): {
  ...
  try {
    ...
    const maybeProduct =
      parsed && typeof parsed === 'object' && (parsed as any).data ? (parsed as any).data : null;
    if (
      maybeProduct &&
      typeof maybeProduct === 'object' &&
      (maybeProduct as any)['@type'] === 'Product'
    ) {
  ...
  if (productJsonLd && typeof productJsonLd === 'object') {
    ...
    if (p.offers) {
      ...
      if (offers && typeof offers === 'object') {
        ...
        const priceNum =
          typeof price === 'string' ? Number.parseInt(price.replace(/\D/g, ''), 10) : price;
        if (Number.isFinite(priceNum)) {
          productPatch.price = {
            current: priceNum,
            old: null,
            currency:
              typeof (offers as any).priceCurrency === 'string'
                ? (offers as any).priceCurrency
                : 'RUB',
          };
        }
        if (typeof (offers as any).availability === 'string')
          productPatch.availability = (offers as any).availability;
      }
    }
    if (p.aggregateRating && typeof p.aggregateRating === 'object') {
      ...
      productPatch.rating = {
        value: Number.isFinite(rv) ? rv : null,
        count: Number.isFinite(rc) ? rc : null,
      };
    }
...
function parseDnsCharacteristicsHtml(characteristicsHtml: string): SpecItem[] {
  ...
  $('.product-characteristics__group').each((_, groupEl) => {
    const group = $(groupEl)
      .find('.product-characteristics__group-title')
      .first()
      .text()
      .replace(/\s+/g, ' ')
      .trim();
...
async function main() {
  ...
  const urlWithCity = productUrl.includes('?')
    ? `${productUrl}&cityPath=msk`
    : `${productUrl}?cityPath=msk`;
  ...
  const baseResp = await page.goto('https://www.dns-shop.ru/?cityPath=msk', {
  ...
  const productResp = await page.goto(urlWithCity, {
    waitUntil: 'domcontentloaded',
    timeout: 60000,
  });
  await page.waitForTimeout(4000);
  ...
  if (charPath) {
    ...
    const m = charHtml.match(
      /\\\/catalog\\\/product\\\/get-product-characteristics-actual\\\/\?id=[0-9a-f-]+/i
    );
    if (m?.[0]) {
